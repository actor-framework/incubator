...
struct fixture : host_fixture {
  ...
  void rfc6455_append(uint8_t opcode, string_view text, byte_buffer& out,
                      uint8_t flags = detail::rfc6455::fin_flag) {
...
CAF_TEST(data may arrive fragmented) {
  ...
  CHECK_EQ(transport.handle_input(),
           static_cast<ptrdiff_t>(opening_handshake.size()));
  byte_buffer buf;
  rfc6455_append(detail::rfc6455::text_frame, "Hello "_sv, buf, 0);
  rfc6455_append(detail::rfc6455::continuation_frame, "WebSocket!\n"_sv, buf);
  rfc6455_append(detail::rfc6455::text_frame, "Bye "_sv, buf, 0);
  rfc6455_append(detail::rfc6455::continuation_frame, "Web"_sv, buf, 0);
